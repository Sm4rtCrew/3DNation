...
interface FormState {
  name: string; credit_limit: string; closing_day: string; due_day: string;
  allow_overlimit: boolean; overlimit_limit: string;
}
const defaultForm: FormState = {
  name: "", credit_limit: "", closing_day: "25", due_day: "5", allow_overlimit: false, overlimit_limit: "0",
};
...
function CreditBar({ used, limit }: { used: number; limit: number }) {
  ...
  return (
    <div className="space-y-1">
      <div className="flex justify-between text-[10px] text-muted-foreground"><span>Disponible</span><span>{Math.round(pct)}%</span></div>
...
export default function FinanceCards() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    const { data } = await supabase.from("cards").select("*").eq("business_id", activeBusiness.id);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`cards-${activeBusiness.id}`)
      .on("postgres_changes", { event: "*", schema: "public", table: "cards", filter: `business_id=eq.${activeBusiness.id}` }, () => load())
      .subscribe();
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!limit || limit <= 0) { setFormError("El límite debe ser mayor a 0."); return; }
    if (!activeBusiness) return;
  ...
  return (
    <div className="p-6 space-y-5 max-w-7xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {cards.map((card) => {
            const used = Number(card.credit_limit) - Number(card.available_credit);
            const isOverlimit = Number(card.available_credit) < 0;
            return (
              <div key={card.id} className="glass-card rounded-2xl p-5 flex flex-col gap-4 fade-in neon-border">
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div><p className="text-[10px] text-muted-foreground uppercase tracking-widest">Disponible</p>
                    <p className={`font-bold font-mono-finance text-lg ${isOverlimit ? "stat-negative" : "stat-positive"}`}>{formatCOP(Number(card.available_credit))}</p>
                  </div>
                  <div><p className="text-[10px] text-muted-foreground uppercase tracking-widest">Utilizado</p>
                    <p className="font-bold font-mono-finance text-lg">{formatCOP(used)}</p>
                  </div>
                  <div><p className="text-[10px] text-muted-foreground uppercase tracking-widest">Límite</p>
                    <p className="font-medium font-mono-finance">{formatCOP(Number(card.credit_limit))}</p>
                  </div>
                  <div><p className="text-[10px] text-muted-foreground uppercase tracking-widest">Cierre / Pago</p>
                    <p className="font-medium">Día {card.closing_day} / {card.due_day}</p>
                  </div>
                </div>
                <CreditBar used={used} limit={Number(card.credit_limit)} />
              </div>
            );
          })}
        </div>
          ...
          <div className="glass-card rounded-2xl w-full max-w-md max-h-[90vh] overflow-y-auto slide-up">
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              <div className="space-y-1.5"><Label>Nombre *</Label>
                <Input placeholder="Visa Bancolombia..." value={form.name}
                  onChange={(e) => setForm(f => ({ ...f, name: e.target.value }))} required className="bg-muted border-border" />
              </div>
              <div className="space-y-1.5"><Label>Límite de Crédito (COP) *</Label>
                <Input type="number" min="1000" step="1000" placeholder="5000000" value={form.credit_limit}
                  onChange={(e) => setForm(f => ({ ...f, credit_limit: e.target.value }))} required className="bg-muted border-border" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5"><Label>Día Cierre *</Label>
                  <Input type="number" min="1" max="31" value={form.closing_day}
                    onChange={(e) => setForm(f => ({ ...f, closing_day: e.target.value }))} required className="bg-muted border-border" />
                </div>
                <div className="space-y-1.5"><Label>Día Pago *</Label>
                  <Input type="number" min="1" max="31" value={form.due_day}
                    onChange={(e) => setForm(f => ({ ...f, due_day: e.target.value }))} required className="bg-muted border-border" />
                </div>
              </div>
              <div className="flex items-center gap-3 p-3 rounded-xl bg-muted">
                <input type="checkbox" id="overlimit" checked={form.allow_overlimit}
                  onChange={(e) => setForm(f => ({ ...f, allow_overlimit: e.target.checked }))} className="rounded" />
                <label htmlFor="overlimit" className="text-sm cursor-pointer">Permitir sobrecupo</label>
              </div>
              {form.allow_overlimit && (
                <div className="space-y-1.5"><Label>Límite Sobrecupo (COP)</Label>
                  <Input type="number" min="0" step="1000" value={form.overlimit_limit}
                    onChange={(e) => setForm(f => ({ ...f, overlimit_limit: e.target.value }))} className="bg-muted border-border" />
                </div>
              )}
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <Button type="submit" disabled={saving} style={{ background: "hsl(var(--primary))", color: "hsl(var(--primary-foreground))" }}>
