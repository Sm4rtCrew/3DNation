...
const PRESET_COLORS = ["#ef4444","#f97316","#f59e0b","#84cc16","#22c55e","#06b6d4","#3b82f6","#8b5cf6","#ec4899","#ffffff"];
...
export default function FinanceCategories() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    const { data } = await supabase.from("finance_categories").select("*").eq("business_id", activeBusiness.id).order("name");
    setCats((data as any) || []);
    setLoading(false);
  }, [activeBusiness]);
  ...
  const openEdit = (cat: FinanceCategory) => {
    setEditingCat(cat);
    setForm({ name: cat.name, color: cat.color, icon: cat.icon || "", parent_id: cat.parent_id || "" });
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const payload = {
      name: form.name.trim(), color: form.color, icon: form.icon || null,
      parent_id: form.parent_id || null, business_id: activeBusiness.id,
    };

    let error;
    if (editingCat) {
      ({ error } = await supabase.from("finance_categories").update(payload).eq("id", editingCat.id));
    } else {
      ({ error } = await supabase.from("finance_categories").insert(payload));
    }

    if (error) setFormError(error.message);
    else { setShowModal(false); setForm(defaultForm); setEditingCat(null); load(); }
    setSaving(false);
  };

  const handleDelete = async (cat: FinanceCategory) => {
    if (!confirm(`¿Eliminar categoría "${cat.name}"?`)) return;
    await supabase.from("finance_categories").delete().eq("id", cat.id);
    load();
  };
            ...
            return (
              <div key={root.id} className="glass-card rounded-2xl overflow-hidden fade-in">
                <div className="flex items-center gap-3 px-5 py-4 border-b border-border/30">
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0" style={{ background: `${root.color}22` }}>
                    <span className="text-sm" style={{ color: root.color }}>{root.icon || <Tag className="w-4 h-4" />}</span>
                  </div>
                  ...
                  <div className="flex items-center gap-1">
                    <button onClick={() => openEdit(root)} className="p-1.5 rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted transition-colors"><Pencil className="w-3.5 h-3.5" /></button>
                    <button onClick={() => handleDelete(root)} className="p-1.5 rounded-lg text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                  </div>
                </div>
                {subs.map((sub) => (
                  <div key={sub.id} className="flex items-center gap-3 px-5 py-3 border-b border-border/20 last:border-0 bg-muted/20">
                    ...
                    <div className="flex items-center gap-1">
                      <button onClick={() => openEdit(sub)} className="p-1 rounded text-muted-foreground hover:text-foreground"><Pencil className="w-3 h-3" /></button>
                      <button onClick={() => handleDelete(sub)} className="p-1 rounded text-muted-foreground hover:text-destructive"><Trash2 className="w-3 h-3" /></button>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm fade-in">
          <div className="glass-card rounded-2xl w-full max-w-md slide-up">
            <div className="flex items-center justify-between px-6 py-4 border-b border-border/50">
              ...
              <button onClick={() => { setShowModal(false); setEditingCat(null); }}><X className="w-5 h-5 text-muted-foreground" /></button>
            </div>
            <form onSubmit={handleSave} className="p-6 space-y-4">
              <div className="space-y-1.5"><Label>Nombre *</Label>
                <Input placeholder="Alimentación, Transporte..." value={form.name}
              ...
              <div className="space-y-1.5"><Label>Emoji (opcional)</Label>
                <Input placeholder="🍔 💰 🚗" value={form.icon} maxLength={4}
                  onChange={(e) => setForm(f => ({ ...f, icon: e.target.value }))} className="bg-muted border-border" />
              </div>
              <div className="space-y-1.5"><Label>Categoría Padre</Label>
                <select value={form.parent_id} onChange={(e) => setForm(f => ({ ...f, parent_id: e.target.value }))}
                  className="w-full px-3 py-2 rounded-lg text-sm bg-muted border border-border text-foreground">
                  <option value="">Ninguna (raíz)</option>
                  {roots.filter(r => r.id !== editingCat?.id).map(r => <option key={r.id} value={r.id}>{r.name}</option>)}
                </select>
              </div>
