...
export default function FinanceLogin() {
  const { login, register, loading, token } = useFinanceAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "login") {
        ...
        setSuccess("Revisa tu correo para confirmar la cuenta. Luego podrás iniciar sesión.");
        setMode("login");
        setPassword("");
      }
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-finance-gradient flex items-center justify-center p-4 relative overflow-hidden">
      ...
      {/* Glow orbs */}
      <div className="absolute top-1/4 -left-32 w-96 h-96 rounded-full blur-3xl opacity-10" style={{ background: "hsl(var(--primary))" }} />
      <div className="absolute bottom-1/4 -right-32 w-96 h-96 rounded-full blur-3xl opacity-5" style={{ background: "hsl(var(--primary))" }} />
      ...
      <div className="w-full max-w-md relative z-10 fade-in">
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground mt-1 text-sm">Plataforma Contable Empresarial</p>
        </div>
        ...
        <div className="glass-card rounded-2xl p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            {mode === "register" && (
              <div className="space-y-2 slide-up">
                ...
                <div className="relative">
                  ...
                  <Input id="fullName" placeholder="Juan Pérez" value={fullName}
            ...
            <Button type="submit" disabled={loading} className="w-full font-semibold py-5 text-sm neon-red"
          ...
          <p className="text-xs text-muted-foreground text-center mt-6">Conexión segura · Lovable Cloud</p>
        </div>

        <div className="text-center mt-6 text-xs text-muted-foreground">Sistema Contable v1.0 · Moneda COP</div>
      </div>
    </div>
  );
}
